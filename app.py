...
class AppHorarios(ctk.CTk):
    ...
    def _construir_ui(self):
        ...
        ctk.CTkFrame(sidebar, height=1, fg_color="#1A3F7A").pack(
        ...
        ctk.CTkButton(
            nav_frame, text="◀", width=32, height=28, command=self._mes_anterior
        ).pack(side="left")
        ...
        ctk.CTkButton(
            nav_frame, text="▶", width=32, height=28, command=self._mes_siguiente
        ).pack(side="left")
        ctk.CTkButton(
            nav_frame,
            text="Hoy",
        ...
        for i, (key, titulo) in enumerate(
            ...
            ctk.CTkLabel(
                card, text=titulo, font=ctk.CTkFont(size=11), text_color=C_GRAY600
        ...
        ctk.CTkButton(
            ...
            width=90,
            fg_color="white",
            hover_color=C_BLUE_LIGHT,
            border_width=1,
            border_color=C_GRAY200,
            text_color=C_GRAY800,
            command=self._editar_empleado,
        ...
        style.configure(
            ...
            foreground=C_GRAY800,
        )
        style.configure(
            "Custom.Treeview.Heading",
            font=("Segoe UI", 12, "bold"),
            background=C_BLUE_FAINT,
            foreground=C_GRAY600,
            relief="flat",
            padding=(0, 6),
        )
        style.map(
            "Custom.Treeview",
            background=[("selected", C_BLUE_LIGHT)],
            foreground=[("selected", C_NAVY)],
        )

        COLS_TABLA = [c for c in COLS if c != "Observaciones"]
        COLS_WIDTHS_TABLA = [
            w for c, w in zip(COLS, COL_WIDTHS) if c != "Observaciones"
        ]
        ...
        for col, w in zip(COLS_TABLA, COLS_WIDTHS_TABLA):
            self.tree.heading(col, text=col)
            anchor = "w" if col == "Empleado" else "center"
        ...
        self.tree.bind(
            "<Double-1>",
            lambda e: self._editar_empleado() if self.tree.selection() else None,
        )
        self.tree.bind(
            "<Return>",
            lambda e: self._editar_empleado() if self.tree.selection() else None,
        ...
        self.tree.bind(
            "<Delete>",
            lambda e: self._eliminar_empleado() if self.tree.selection() else None,
        )
        ...
        self.lbl_panel_emp = ctk.CTkLabel(
            header_obs,
            text="",
            font=ctk.CTkFont(size=11),
    ...
    def _refrescar_stats(self):
        ...
        self.stat_labels["adelantos"].configure(
            text=f"${t['adelantos']:,.0f}".replace(",", ".")
        )
    ...
    def _nuevo_empleado(self):
        ...
        if dlg.result:
            if db_agregar_empleado(
                dlg.result["Empleado"], self.mes_actual, self.anio_actual
            ):
                self._cargar_mes(self.mes_actual, self.anio_actual)
    ...
    def _empleado_seleccionado(self):
        ...
        idx = next(
            (i for i, e in enumerate(self.empleados) if e["Empleado"] == nombre), None
        )
        return idx, item
    ...
    def _editar_empleado(self):
        ...
        dlg = DialogEmpleado(
            self, f"Editar — {emp_original['Empleado']}", datos=emp_original
        )
        self.wait_window(dlg)
        if dlg.result:
    ...
    def _importar_excel(self):
        ...
        if not datos:
            messagebox.showwarning(
                "Sin datos", "No se encontraron datos en el archivo."
            )
            return
        ...
        for emp in datos:
            nombre = emp["Empleado"]
            cur.execute(
                "INSERT OR IGNORE INTO empleados (nombre) VALUES (?)", (nombre,)
            )
            emp_id = cur.execute(
            ...
            if resp:
                cur.execute(
                    "DELETE FROM planillas WHERE empleado_id=? AND mes=? AND anio=?",
                    (emp_id, self.mes_actual, self.anio_actual),
                )
            cur.execute(
    ...
    def _construir_tabla_anual(self):
        ...
        ctk.CTkLabel(
            frame,
            text=f"Resumen anual {self.anio_actual}",
        ...
        for m in range(1, 13):
            ...
            tiene_datos = any(
                e["Horas"] or e["Hrs. Extras"] or e["Adelanto"] for e in datos
            )
            if not tiene_datos:
                tree.insert(
                    "",
    ...
    def _construir_ranking(self):
        ...
        if not ranking:
            ctk.CTkLabel(
                frame, text="Sin datos numéricos de horas este mes.", text_color="gray"
        ...
        for i, (nombre, horas) in enumerate(ranking):
            ...
            barra_bg = ctk.CTkFrame(
                scroll_frame, height=18, fg_color=("gray88", "gray25"), corner_radius=4
            )
            ...
            ctk.CTkLabel(
                ...
                anchor="e",
                font=ctk.CTkFont(size=12),
            ).grid(row=i, column=2, pady=4, sticky="e")
